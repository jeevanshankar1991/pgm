...
def gradient():
     ...
     for itr in range(N):
	     word = words[itr]
	     f = 'data/test_img' + str(itr) + '.txt' 
             do_sumproduct(f, word)
	     ## do for transition counts
	     for j in range(len(word)-1):
		      y1 = labels[word[j]]
		      y2 = labels[word[j+1]]
		      trans_data_count[y1][y2] += 1
	     for i in range(nclique): 
		      for y1 in range(n): 
			  for y1 in range(n):
		      trans_expected_count[y1][y2]  += marginal_prob[i][y1][y2] ## marginal_prob is pre-computed as result of sum-product
	     ## do for emission counts
	     for j in range(len(word)):
		     c = labels[word[j]]
		     for f in range(features[j]):
		          emis_data_count[c][f] += features[j][f]
		          for y in range(n):
				  emis_expected_count[y][f] += features[j][f] * single_var[j][y] ## single_var prob is pre-computed as result of sum-product
     k = 0
     for c in range(n):
     ...
     for c in range(n):
	     for f in range(size):
		     grad[i] = emis_data_count[c][f] - emis_transition_count[c][f]
		     k += 1
     return grad
